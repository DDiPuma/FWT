...
def two_dim_forward_benchmark():
    ...
    for data_size in data_sizes:
        ...
        for trial in range(10):
            data_array = np.random.randint(256, size=(data_size,data_size))
...
def fast_two_dim_forward_benchmark():
    ordered_time_averages = []
    inplace_time_averages = []
    ...
    fig = plt.figure(1)
    plt.suptitle("2D Fast Haar Transform Algorithm Runtimes")
    fig.subplots_adjust(left=0.2, top=0.8, wspace=1)
...
def fast_two_dim_inverse_benchmark():
    ordered_time_averages = []
    inplace_time_averages = []
    ...
    fig = plt.figure(1)
    plt.suptitle("2D Fast Inverse Haar Transform Algorithm Runtimes")
    fig.subplots_adjust(left=0.2, top=0.8, wspace=1)
